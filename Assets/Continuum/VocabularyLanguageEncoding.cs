...
/// <see cref="BuiltInUrnPrefix"/> (operational rule; matches continuum <c>thesaurus_entries.id</c> TEXT PK).
/// </summary>
public static class VocabularyLanguageEncoding
{
    /// <summary>Operational rule: continuum ETL / API must not emit primary keys with this prefix.</summary>
    public const string BuiltInUrnPrefix = "urn:unity:continuum:builtin:v1:";
    ...
    /// <summary>
    /// Resolve <paramref name="languageCode"/> to continuum <c>languages.id</c> using caller-supplied lookup
    /// (e.g. <c>SELECT id FROM languages WHERE code = ?</c>). Same contract as continuum_api server.
    /// </summary>
