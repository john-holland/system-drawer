...
/// <summary>Editor prefab hierarchy scanner — posts component blueprints to Continuum API.</summary>
public static class LemmaComponentBlueprintScanner
{
    ...
    public static async Task<bool> PostBlueprintAsync(string entryId, ComponentMetadataPayloadDto payload, CancellationToken ct = default)
    {
        ...
        var r = await ContinuumEditorApiClient.RequestAsync("POST", path, body, ct);
        if (!r.success)
    ...
    [MenuItem("Continuum/Scan lemma prefab blueprints…")]
