...
public sealed class ContinuumNotificationsWindow : EditorWindow
{
    static ContinuumNotificationFeedController _feed = new ContinuumNotificationFeedController();
    ...
    [MenuItem("Window/Continuum/Notifications")]
    public static void Open()
    {
        var w = GetWindow<ContinuumNotificationsWindow>("Continuum Notifications");
    ...
    {
        _badgeCount = count;
        var w = Resources.FindObjectsOfTypeAll<ContinuumNotificationsWindow>().FirstOrDefault();
        w?.Repaint();
    }
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("Hub", EditorStyles.toolbarButton, GUILayout.Width(40)))
            Application.OpenURL(ContinuumEditorSession.ApiBaseUrl + "/ui");
    ...
    void DrawFeed()
    {
        ...
        foreach (var n in _feed.Items)
        {
            ...
            EditorGUILayout.LabelField(ContinuumNotificationFeedController.TypeLabel(n.type), unread ? EditorStyles.boldLabel : EditorStyles.label);
            ...
            if (!string.IsNullOrEmpty(n.draftId) && GUILayout.Button("Script Editor", GUILayout.Width(90)))
                ContinuumScriptEditorWindow.Open(n.draftId, n.reviewId);
            EditorGUILayout.EndHorizontal();
    ...
    void DrawConsole()
    {
        ...
        foreach (var e in ContinuumNotificationConsoleSink.Entries)
        {
