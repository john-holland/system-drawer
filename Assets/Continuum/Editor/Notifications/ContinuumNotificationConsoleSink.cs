...
/// <summary>Append-only Continuum editor console feed (notifications + local events).</summary>
public static class ContinuumNotificationConsoleSink
{
    ...
    public static void Log(string type, string message, string draftId = null, string reviewId = null)
    {
        ...
        if (_entries.Count > MaxEntries)
            _entries.RemoveAt(0);
        UnityEngine.Debug.Log($"[Continuum] {type}: {message}");
        Changed?.Invoke();
    }
