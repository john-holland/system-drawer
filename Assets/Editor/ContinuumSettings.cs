...
public static class ContinuumSettings
{
    ...
    /// <summary>
    /// DB path for continuum.db. From continuum_db_path.txt in Scripts or empty.
    /// </summary>
    public static string GetDbPath()
    {
        if (_cachedDbPath != null)
            return _cachedDbPath;
        var p = Path.Combine(Application.dataPath, "..", "Scripts", "continuum_db_path.txt");
        _cachedDbPath = File.Exists(p) ? File.ReadAllText(p).Trim() : "";
        return _cachedDbPath;
    }

    /// <summary>
    /// Python path for USC CLI. From continuum_python_path.txt in Scripts or empty (use PATH).
    /// </summary>
    public static string GetPythonPath()
    {
        if (_cachedPythonPath != null)
            return _cachedPythonPath;
        var p = Path.Combine(Application.dataPath, "..", "Scripts", "continuum_python_path.txt");
        _cachedPythonPath = File.Exists(p) ? File.ReadAllText(p).Trim() : "";
        return _cachedPythonPath;
    }

    /// <summary>
    /// Continuum API base URL for notifications. From Scripts/continuum_api_url.txt or http://localhost:5050.
    /// </summary>
