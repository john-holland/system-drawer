// the lower leg has two bones that turn to turn the foot, where the upper leg has a large crutch to catch the lower legs
//   and at the top, a large ball joint like the shoulder, with similar limits although a steeper eliptoid limit on most adults
// the knee can bend anywhere between zero, and 180 to 270

// the top leg front muscles (name?) pull the patella up
// the top leg front muscles pull the bottom leg up
// the top leg back muscles (name?) pull the patella taught
// the top leg back muscles pull the bottom leg in, or down, like a curl
// the bottom leg front muscles lift the foot
// the bottom leg back muscles pull or root the foot
// the bottom right leg side muscles pull the foot to the right
// the bottom left leg side muscles pull the foot to the left
// the top leg side muscles outside pull the leg open, they also help curl the lower leg with a twist, vital for a famous dance move
// the top leg side muscles inside pull the leg closed, they also help curl the lower leg


