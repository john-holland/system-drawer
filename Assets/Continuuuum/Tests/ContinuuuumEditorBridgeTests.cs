...
public class ContinuuuumEditorBridgeTests
{
    [Test]
    ...
    {
        var resp = await ContinuuuumEditorBridge.HandleAsync("{\"action\":\"nope\",\"requestId\":\"t2\"}");
    ...
    {
        var resp = new ContinuuuumEditorBridge.BridgeResponse { requestId = "x", ok = true, data = "{\"a\":1}" };
        var json = ContinuuuumEditorBridge.ToJson(resp);
        var back = JsonUtility.FromJson<ContinuuuumEditorBridge.BridgeResponse>(json);
        Assert.AreEqual("x", back.requestId);
        Assert.IsTrue(back.ok);
    }
}

public class ContinuuuumNotificationClientTests
{
    [Test]
