...
public class ContinuuuumScriptSpanOverlayTests
{
    [Test]
    public void Build_IncludesPromptClauseAndCommentRanges()
    {
        ...
        var spans = ContinuuuumScriptSpanOverlayModel.Build(text, bindings, comments);
        Assert.IsTrue(spans.Any(s => s.kind == ContinuuuumScriptSpanOverlayModel.SpanKind.Prompt));
        Assert.IsTrue(spans.Any(s => s.kind == ContinuuuumScriptSpanOverlayModel.SpanKind.Clause));
        Assert.IsTrue(spans.Any(s => s.kind == ContinuuuumScriptSpanOverlayModel.SpanKind.Comment));
    }
    ...
    public void Build_ResolvesClauseSpanFromFareyWhenCharCacheEmpty()
    {
        ...
        var spans = ContinuuuumScriptSpanOverlayModel.Build(text, bindings, null);
        var clause = spans.Find(s => s.kind == ContinuuuumScriptSpanOverlayModel.SpanKind.Clause);
