...
public sealed class ContinuuuumWebViewSpikeWindow : EditorWindow
{
    string _draftId = "draft-spike";
    string _oldText = "Hello {P:name}";
    string _newText = "Hello {P:name|formal=true}";
    string _log = "";
    ContinuuuumWebViewHost _host;
    bool _useWebView = true;

    [MenuItem("Window/Continuuuum/WebView Spike")]
    public static void Open()
    {
        var w = GetWindow<ContinuuuumWebViewSpikeWindow>("WebView Spike");
    ...
    void OnGUI()
    {
        EditorGUILayout.LabelField("Continuuuum WebView Spike", EditorStyles.boldLabel);
        EditorGUILayout.HelpBox(
            "Loads local editor-host.html only. API calls route through ContinuuuumEditorBridge (no credentials in browser).",
            MessageType.Info);

        _useWebView = EditorGUILayout.Toggle("Try WebView embed", _useWebView && ContinuuuumWebViewHost.IsAvailable);
        if (!ContinuuuumWebViewHost.IsAvailable)
        ...
        if (_useWebView && ContinuuuumWebViewHost.IsAvailable)
        {
            ...
            if (_host == null)
            {
                _host = ContinuuuumWebViewHost.TryCreate(hostRect, OnWebMessage);
    ...
    async void RunBridgeApplyEdit()
    {
        ...
        var reqJson = JsonUtility.ToJson(new ContinuuuumEditorBridge.BridgeRequest
        {
            action = "api",
            requestId = "spike-1",
            method = "POST",
        ...
        var resp = await ContinuuuumEditorBridge.HandleAsync(reqJson);
        _log += $"apply-edit ok={resp.ok} data={resp.data} err={resp.error}\n";
        _host?.DeliverBridgeResponse(ContinuuuumEditorBridge.ToJson(resp));
        Repaint();
    }
    ...
    {
        var resp = await ContinuuuumEditorBridge.HandleAsync(json);
        _host?.DeliverBridgeResponse(ContinuuuumEditorBridge.ToJson(resp));
        Repaint();
    }
