/* Shared Continuuuum clause selector — unified attach for property / lemma / localization */
(function (global) {
  ...
  function lemmaLibraryEntryUrl(entryId) {
    if (global.ContinuuuumLemmaEntry) return global.ContinuuuumLemmaEntry.entryUrl(entryId);
  ...
  function openLemmaEntryPage(entryId) {
    ...
    if (global.ContinuuuumLemmaEntry) {
      global.ContinuuuumLemmaEntry.open(entryId);
      return;
    }
  ...
  function clauseActionsHtml(prefix, primaryLabel) {
    return `
        <div class="continuuuum-clause-actions" style="margin-top:14px">
          <div style="display:flex;align-items:flex-start;gap:12px;flex-wrap:wrap">
            ...
            <div id="${prefix}-error" class="continuuuum-clause-dialog-error" role="alert" hidden
              style="flex:1 1 180px;font-size:13px;line-height:1.4;color:#b00020;min-width:0"></div>
          </div>
          <div id="${prefix}-conflict-actions" class="continuuuum-clause-conflict-actions">
  ...
  async function applyBindingEditOrSuggest(draftId, body, options) {
    ...
    const callApi = options.callApi || ((method, path, reqBody) => {
      if (global.ContinuuuumClauseSelector && global.ContinuuuumClauseSelector.callApi) {
        return global.ContinuuuumClauseSelector.callApi(method, path, reqBody);
      }
      return fetch(path, {
        method: method || 'GET',
  ...
  function clearClauseDialogError(box) {
    if (!box) return;
    box.querySelectorAll('.continuuuum-clause-dialog-error').forEach((el) => {
      el.hidden = true;
      el.innerHTML = '';
    });
    box.querySelectorAll('.continuuuum-clause-conflict-actions').forEach((el) => {
  ...
  function setupLemmaPicker(scope, callApi, opts) {
    ...
    if (!opts.prefabOnly && global.ContinuuuumLemmaCompositionEditor && scope.querySelector('#clause-create-mode-tabs')) {
      scope._compositionCreateTabs = global.ContinuuuumLemmaCompositionEditor.mountCreateTabs(
  ...
  const ContinuuuumClauseSelector = {
    fromEditorSelection(editorInst) {
      const sel = global.ContinuuuumScriptEditor
        ? global.ContinuuuumScriptEditor.getSelection(editorInst)
        : { charStart: 0, charEnd: 0, text: '' };
      const text = sel.text || '';
      const n = Math.max((global.ContinuuuumScriptEditor?.getValue(editorInst) || '').length, 1);
    ...
    async callApi(method, path, body) {
      if (global.ContinuuuumScriptEditor?.callApi) {
        try {
          return await global.ContinuuuumScriptEditor.callApi(method, path, body);
        } catch (e) {
    ...
    openAttachDialog(clauseRef, options) {
      ...
      overlay.className = 'continuuuum-clause-overlay';
      ...
      const ensureLemmaPicker = () => {
        ...
        box._lemmaPicker = setupLemmaPicker(box, ContinuuuumClauseSelector.callApi.bind(ContinuuuumClauseSelector), {
      ...
      const useExistingEntry = async (entryId) => {
        ensureLemmaPicker();
        const entry = await ContinuuuumClauseSelector.callApi(
          'GET',
      ...
      ContinuuuumClauseSelector.callApi('GET', '/api/thesaurus/property-specs').then(data => {
      ...
      box.querySelector('#clause-attach-save').onclick = async () => {
        ...
        try {
          ...
          if (activeTab === 'property') {
            ...
            body.entryId = await resolveLemmaEntryId(box, ContinuuuumClauseSelector.callApi.bind(ContinuuuumClauseSelector));
          ...
          await ContinuuuumClauseSelector.callApi('POST', '/api/thesaurus/clause-bindings', body);
          overlay.remove();
      ...
      box.querySelector('#clause-edit-save').onclick = async () => {
        ...
        try {
          ...
          const result = await applyBindingEditOrSuggest(draftId, body, {
            ...options,
            callApi: ContinuuuumClauseSelector.callApi.bind(ContinuuuumClauseSelector),
          });
          overlay.remove();
    ...
    openLemmaEntryDialog(options) {
      ...
      const callApi = options.callApi || ContinuuuumClauseSelector.callApi.bind(ContinuuuumClauseSelector);
    ...
    async cloneBindingTemplates(templates, clauseRef, options) {
      ...
      for (const tpl of templates) {
        const kind = tpl.bindingKind || tpl.binding_kind || 'property';
        await ContinuuuumClauseSelector.callApi('POST', '/api/thesaurus/clause-bindings', {
          ...bodyBase,
          bindingKind: kind,
  ...
  global.ContinuuuumClauseSelector = ContinuuuumClauseSelector;
  global.ContinuuuumClauseSelectorBindingKinds = BINDING_KINDS;
