...
public sealed class ContinuuuumWebViewHost : IDisposable
{
    ...
    static ContinuuuumWebViewHost()
    {
        IsAvailable = Type.GetType("UnityEditor.WebView, UnityEditor") != null
                      || Type.GetType("UnityEditor.MacWebView, UnityEditor") != null;
    }

    ContinuuuumWebViewHost(object webView, MethodInfo loadUrl, MethodInfo executeJs, Action<string> onMessage)
    {
        _webView = webView;
        _loadUrl = loadUrl;
    ...
    public static ContinuuuumWebViewHost TryCreate(Rect hostRect, Action<string> onMessageFromJs)
    {
        ...
        try
        {
            ...
            if (loadUrl == null)
                return null;
            return new ContinuuuumWebViewHost(instance, loadUrl, executeJs, onMessageFromJs);
        }
        catch (Exception ex)
        {
            Debug.LogWarning($"[Continuuuum] WebView create failed: {ex.Message}");
            return null;
        }
    }
    ...
    public void LoadBundledHost()
    {
        ...
        if (!File.Exists(path))
        {
            Debug.LogWarning($"[Continuuuum] Missing bundled editor-host.html at {path}");
            return;
        }
    ...
    public void MountEditor(string scriptText, bool readOnly, string draftEpisodeId = null, string draftScriptId = null)
    {
        ...
        string js = $@"window.continuuuumHost && window.continuuuumHost.mount({{
          scriptText: '{escaped}',
    ...
    {
        if (_executeJs == null) return;
        _executeJs.Invoke(_webView, new object[] { "window.continuuuumHost && window.continuuuumHost._inst && ContinuuuumScriptEditor.markMayorDogModSlot(window.continuuuumHost._inst);" });
    }
    ...
    {
        string rel = "Assets/Continuuuum/Editor/ScriptEditor/WebView/editor-host.html";
