...
/// <summary>Editor for lemma-level localization property bags (syncs with Continuuuum API).</summary>
public sealed class VocabularyLemmaPropertyEditorWindow : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        {
            var baseUrl = ContinuuuumEditorSession.ApiBaseUrl.TrimEnd('/');
    ...
    async Task LoadCompositionAsync()
    {
        ...
        var data = await ContinuuuumEditorLocalizationClient.Instance.GetCompositionAsync(_entryId);
    ...
    async Task SearchAndAddCompositionChildAsync()
    {
        ...
        var r = await ContinuuuumEditorLocalizationClient.Instance.CallRawAsync("GET", $"/api/thesaurus/entries?q={q}&limit=1", null);
        ...
        var saved = await ContinuuuumEditorLocalizationClient.Instance.PutCompositionAsync(_entryId, put);
    ...
    async Task RemoveCompositionChildAsync(string childEntryId)
    {
        ...
        var list = (_compositionChildren ?? Array.Empty<LemmaCompositionChildDto>())
            ...
            .ToArray();
        var saved = await ContinuuuumEditorLocalizationClient.Instance.PutCompositionAsync(_entryId, list);
    ...
    async Task RecombobulateSpatialAsync()
    {
        ...
        {
            var draft = await ContinuuuumEditorLocalizationClient.Instance.GetDraftScriptAsync(_clauseDraftId);
            scriptText = draft?.scriptText ?? "";
        }
        var audit = await ContinuuuumEditorLocalizationClient.Instance.RecombobulateSpatialAsync(
            _entryId,
        ...
        await ContinuuuumEditorLocalizationClient.Instance.RecombobulateSpatialAsync(
            _entryId,
    ...
    {
        if (string.IsNullOrEmpty(_entryId)) return;
        var client = ContinuuuumEditorLocalizationClient.Instance;
    ...
    {
        var specs = await ContinuuuumEditorLocalizationClient.Instance.GetPropertySpecsAsync();
