...
public static class ContinuuuumEditorBridge
{
    [Serializable]
    ...
    public static async Task<BridgeResponse> HandleAsync(string json)
    {
        ...
        {
            case "api":
                var api = await ContinuuuumEditorApiClient.RequestAsync(req.method ?? "GET", req.path, req.body);
            ...
            case "openReview":
                ContinuuuumScriptEditorWindow.Open(req.draftId, req.reviewId);
                return new BridgeResponse { requestId = req.requestId, ok = true };
            case "notificationRead":
                await ContinuuuumEditorLocalizationClient.Instance.MarkReadAsync(req.notificationId);
                return new BridgeResponse { requestId = req.requestId, ok = true };
            case "scriptChanged":
                return new BridgeResponse
                {
