...
/// <summary>
/// Buffers runtime component-creation reports and pushes them to Continuuuum in dev mode.
/// </summary>
public static class LemmaComponentReportCollector
{
    ...
    {
        if (!ContinuuuumApiConfig.ShouldPushRuntimeComponentReports || string.IsNullOrEmpty(entryId) || instance == null)
            return;
        try
        {
    ...
    {
        if (!ContinuuuumApiConfig.ShouldPushRuntimeComponentReports)
            return;
    ...
    {
        if (ContinuuuumApiConfig.DisableReportingForSkunkWorks || payload == null || string.IsNullOrEmpty(entryId))
            return false;
        payload.entryId = entryId;
        payload.source = "runtime";
        payload.runId = payload.runId ?? CurrentRunId;
        var url = ContinuuuumApiConfig.GetApiBaseUrl().TrimEnd('/') +
