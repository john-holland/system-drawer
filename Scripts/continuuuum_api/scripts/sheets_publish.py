...
CONTINUUUUM_DB = os.environ.get("CONTINUUUUM_DB", str(Path(__file__).resolve().parents[2] / "continuuuum.db"))
...
def _load_stories_for_plan(plan_id: str) -> list[dict]:
    conn = sqlite3.connect(CONTINUUUUM_DB)
...
def budget_template_payload() -> dict[str, Any]:
    ...
    return {
        "ok": True,
        "version": 1,
        "description": "Continuuuum production budget template (Google Sheets / publish-sheets compatible)",
        "tabs": tabs,
        ...
        for m in s.get("milestones") or []:
            schedule_tab.append(
                [
                    ...
                    m.get("start_date"),
                    m.get("end_date"),
                    json.dumps(m.get("continuuuum_story_ids") or []),
                ]
            )
