"""Merge built-in vocabulary JSON with continuuuum DB thesaurus rows."""
...
def merge_vocabulary(conn: sqlite3.Connection) -> dict[str, dict[str, Any]]:
    ...
    try:
        from continuuuum_api.lemma_component_metadata import component_creation_view, load_all_cache_maps
    except ImportError:
    ...
    try:
        try:
            from continuuuum_api.lemma_composition import load_all_parent_children
        except ImportError:
    ...
    try:
        try:
            from continuuuum_api.lemma_prompt import ensure_lemma_prompt_schema, load_overlay
        except ImportError:
