...
MODS_SCHEMA = REPO_ROOT / "continuuuum_mayor_dog_mods_schema.sql"
...
def iter_m_slots(text: str) -> list[tuple[str, str | None, int, int]]:
    """Return (slot_key, default_label, char_start, char_end) for each {M:key} in text."""
    out: list[tuple[str, str | None, int, int]] = []
    if not text:
        return out
    for m in M_SLOT_RE.finditer(text):
        key = (m.group(1) or m.group(3) or "").strip()
        default = (m.group(2) or m.group(4) or "").strip() or None
        if not key:
            continue
        out.append((key, default, m.start(), m.end()))
    return out
...
def sync_lemma_mod_slots(conn: sqlite3.Connection) -> int:
    """Upsert lemma_prompt moddable targets from {M:slot} markers in lemma prompts."""
    ensure_mayor_dog_mods_schema(conn)
    seen: dict[str, dict[str, Any]] = {}
    if _table_exists(conn, "thesaurus_lemma_overlays"):
        for row in conn.execute(
            "SELECT target_entry_id, lemma_prompt FROM thesaurus_lemma_overlays "
            "WHERE lemma_prompt IS NOT NULL AND lemma_prompt LIKE '%M:%'"
        ).fetchall():
            for key, default, start, end in iter_m_slots(row["lemma_prompt"] or ""):
                if key in seen:
                    continue
                seen[key] = {
                    ...
                    "slotKey": key,
                    "label": default or key,
                    "charStart": start,
                    "charEnd": end,
                    "sourceText": row["lemma_prompt"] or "",
                }
    if _table_exists(conn, "thesaurus_entry_properties"):
        for row in conn.execute(
            "SELECT entry_id, property_value FROM thesaurus_entry_properties "
            "WHERE property_key = 'lemma-prompt' AND property_value LIKE '%M:%'"
        ).fetchall():
            for key, default, start, end in iter_m_slots(row["property_value"] or ""):
                if key in seen:
                    continue
                seen[key] = {
                    "targetKind": "lemma_prompt",
                    ...
                    "charStart": start,
                    "charEnd": end,
                    "sourceText": row["property_value"] or "",
                }
    n = 0
    for body in seen.values():
        upsert_moddable_target(conn, body)
        n += 1
    return n


def sync_episode_mod_slots(conn: sqlite3.Connection, draft_episode_id: str) -> dict[str, Any]:
    """Upsert episode_section targets from {M:slot} in a draft script. Returns sync meta."""
    ensure_mayor_dog_mods_schema(conn)
    meta: dict[str, Any] = {
        "draftEpisodeId": draft_episode_id,
        "scriptLength": 0,
        "synced": 0,
        "found": False,
    }
    if not draft_episode_id or not _table_exists(conn, "draft_episode_script"):
        return meta
    row = conn.execute(
        """SELECT script_text FROM draft_episode_script
    ...
    if not row:
        return meta
    text = row["script_text"] or ""
    meta["found"] = True
    meta["scriptLength"] = len(text)
    n = 0
    for key, default, start, end in iter_m_slots(text):
        # slot_key is globally unique — namespace episode slots
        slot_key = key if key.startswith("ep-") else f"ep-{draft_episode_id[:8]}-{key}"
...
def upsert_moddable_target(conn: sqlite3.Connection, body: dict[str, Any]) -> dict[str, Any]:
    ...
    if not existing:
        by_slot = conn.execute(
            "SELECT id FROM moddable_targets WHERE slot_key = ?", (slot_key,)
        ).fetchone()
        if by_slot:
            target_id = by_slot["id"]
            existing = by_slot
    if not target_id:
        target_id = _new_id("mtgt")
