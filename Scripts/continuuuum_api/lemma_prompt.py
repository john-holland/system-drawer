...
try:
    from continuuuum_api.mod_db import build_mod_context_from_manifest, resolve_mod_placeholders
except ImportError:
...
try:
    from continuuuum_api.lemma_composition import (
    ...
    from continuuuum_api.lemma_merge import is_builtin_urn, merge_vocabulary
except ImportError:
...
try:
    from continuuuum_api.lemma_composition_spatial import upsert_entry_spatial
except ImportError:
...
def ensure_lemma_prompt_schema(conn: sqlite3.Connection) -> None:
    ...
    if not _table_exists(conn, "thesaurus_lemma_overlays"):
        sql = (_SCHEMA_ROOT / "continuuuum_lemma_prompt_schema.sql").read_text(encoding="utf-8")
...
def expand_lemma_prompt(
    ...
    *,
    ...
    if "{P:dialogue" in template.lower():
        try:
            from continuuuum_api.dialogue_parser import compile_dialogue_to_json
        except ImportError:
    ...
    if "{P:quest" in template.lower():
        try:
            from continuuuum_api.quest_parser import compile_quest_to_json
        except ImportError:
