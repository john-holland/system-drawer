...
try:
    ...
    from continuuuum_api.localization_routes import _ensure_review_columns, _load_bindings_for_draft, execute_binding_edit
...
def register_script_output_routes(app, get_conn: GetConn, get_user: GetUser) -> None:
    ...
    @app.route("/api/drafts/episodes/<draft_id>/auto-add-single-lemmas", methods=["POST"])
    def auto_add_single_lemmas_route(draft_id: str):
        body = request.get_json() or {}
        user_id = get_user()
        try:
            try:
                from continuuuum_api.lemma_auto_bind import auto_add_single_lemmas
            except ImportError:
                from lemma_auto_bind import auto_add_single_lemmas
            conn = get_conn()
            ensure_clause_binding_columns(conn)
            draft = conn.execute("SELECT id, committed_at FROM draft_episodes WHERE id = ?", (draft_id,)).fetchone()
            if not draft:
                conn.close()
                return jsonify({"error": "draft not found"}), 404
            if draft["committed_at"]:
                conn.close()
                return jsonify({"error": "draft is committed"}), 409
            auth_err = require_draft_author(conn, draft_id, user_id)
            if auth_err:
                conn.close()
