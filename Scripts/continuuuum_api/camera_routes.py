...
def ensure_camera_tables(conn: sqlite3.Connection) -> None:
    schema_path = Path(__file__).resolve().parents[1] / "continuuuum_camera_schema.sql"
...
def register_camera_routes(app, get_conn: GetConn, get_user: Callable[[], str]) -> None:
    ...
    def camera_comments(scene_id: str):
        ...
        try:
            ...
            if request.method == "GET":
                cur = conn.execute(
                    """SELECT * FROM continuuuum_threaded_comments
            ...
            conn.execute(
                """INSERT INTO continuuuum_threaded_comments
            ...
            conn.commit()
            cur = conn.execute("SELECT * FROM continuuuum_threaded_comments WHERE id = ?", (cid,))
