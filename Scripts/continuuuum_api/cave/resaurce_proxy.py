...
def _local_route_fallback(structural: str, payload: dict[str, Any] | None = None) -> dict[str, Any] | None:
    ...
    if structural == "production/schedule/list":
        ...
        try:
            ...
            try:
                from continuuuum_api.local_production_store import list_schedules as local_list_schedules
            except ImportError:
    ...
    if structural == "production/schedule/create":
        try:
            ...
            conn.row_factory = sqlite3.Row
            try:
                from continuuuum_api.local_production_store import create_schedule as local_create_schedule
            except ImportError:
    ...
    if structural == "production/schedule/milestone/stories":
        ...
        story_ids = (payload or {}).get("continuuuum_story_ids") or (payload or {}).get("story_ids") or []
        try:
            conn = sqlite3.connect(_default_db_path())
            try:
                from continuuuum_api.local_production_store import update_milestone_stories
            except ImportError:
