...
```bash
cd Scripts/continuuuum_api && npm install
python -m continuuuum_api.server --db continuuuum.db --port 5050
```
...
Built-in vocabulary JSON: `continuuuum_api/data/builtin_vocabulary.json` (export via Unity **Continuuuum → Export Built-in Vocabulary JSON**).

Apply `continuuuum_lemma_library_schema.sql` for `prefab-id` property spec and indexes.

## Run

Set continuuuum.db path (or place `continuuuum.db` in repo root):

```bash
pip install -r requirements.txt
CONTINUUUUM_DB=/path/to/continuuuum.db python -m continuuuum_api.server --port 5050
```

Or: `python -m continuuuum_api.server --db /path/to/continuuuum.db`

Apply `continuuuum_episodes_schema.sql`, `continuuuum_thesaurus_schema.sql`, `continuuuum_screenplay_schema.sql`, `continuuuum_draft_schema.sql`, and `continuuuum_review_schema.sql` to the DB first.
