...
# Income inflows that feed Continuuuum company payroll (HWM / retainer) when product is linked.
PAYROLL_INCOME_ENTRY_TYPES = frozenset(
    {
        "preorder_deposit",
        "preorder_backer_stake",
        "investment",
...
def _ledger(conn: sqlite3.Connection, entry_type: str, product_id: str | None, **kw: Any) -> str:
    ...
    if entry_type in PAYROLL_INCOME_ENTRY_TYPES and product_id:
        amt = net if net is not None else gross
        if amt is not None and float(amt) > 0:
            try:
                try:
                    from continuuuum_api.payroll_engine import maybe_post_income_for_product
                except ImportError:
                    from payroll_engine import maybe_post_income_for_product
                maybe_post_income_for_product(
                    conn,
                    product_id,
                    float(amt),
                    source=entry_type,
                    ...
                    meta={"ledgerEntryId": lid, "entryType": entry_type},
                )
            except Exception:
                pass
    return lid
...
def register_saurce_routes(app, get_conn: GetConn, get_current_user: GetUser) -> None:
    ...
    def saurce_positions():
        ...
        if request.method == "POST":
            body = request.get_json(silent=True) or {}
            pid = new_id()
            committed = float(body.get("committedAmount") or 0)
            conn.execute(
                """INSERT INTO saurce_investment_positions
            ...
            if committed > 0:
                _ledger(
                    ...
                    "investment",
                    body["productId"],
                    gross_amount=committed,
                    net_amount=committed,
                    position_id=pid,
                    idempotency_key=f"invest:{pid}",
                )
            conn.commit()
            conn.close()
