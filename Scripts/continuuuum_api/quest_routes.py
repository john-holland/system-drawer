...
def register_quest_routes(app, get_conn: GetConn, get_current_user: Callable[[], str]) -> None:
    try:
        from continuuuum_api.quest_parser import LITTLE_PRINCE_FIXTURE, compile_quest_to_json
        from continuuuum_api.quest_db import (
            activate_objective,
            complete_objective,
            create_session,
            ensure_quest_schema,
        ...
        from continuuuum_api.quest_art import generate_art, generate_summary, inpaint_art, suggest_style
        from continuuuum_api.quest_spatial import list_spatial_nodes
    except ImportError:
