...
How to add or change a Continuuuum API surface using YAML-first Cave routing.
...
```javascript
ContinuuuumCaveShell.init({ tomeId: 'my-surface-tome' });
ContinuuuumCaveShell.caveMessage('my_new_action', { foo: 'bar' });
// or
ContinuuuumCaveShell.caveRoute('continuuuum:my-domain/action', { foo: 'bar' });
// or via RobotCopy
...
r = client.post("/cave/route", json={
    "route": "continuuuum:my-domain/action",
    "payload": {},
    "trace_id": "test_1",
})
...
| `route` | yes* | `continuuuum:stories/list` or `resaurce:chat/message/send` |
