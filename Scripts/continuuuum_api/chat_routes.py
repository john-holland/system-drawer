"""Proxy resaurce Cave chat routes for Continuuuum UI, with local SQLite fallback."""
...
try:
    from continuuuum_api.local_chat_store import (
...
def register_chat_routes(app, get_conn: GetConn) -> None:
    ...
    def chat_ensure_story():
        ...
        if _use_local(out, status):
            story_id = body.get("story_id") or body.get("continuuuum_story_id")
            if not story_id:
    ...
    def chat_ensure_table_read():
        ...
        if _use_local(out, status):
            session_id = body.get("session_id") or body.get("continuuuum_table_read_session_id")
            if not session_id:
    ...
    def chat_room_for_table_read(session_id: str):
        ...
        if _use_local(out, status):
            ...
            row = conn.execute(
                "SELECT * FROM chat_rooms WHERE continuuuum_table_read_session_id = ? LIMIT 1",
                (session_id,),
            ).fetchone()
            conn.close()
            if not row:
