(function () {
  ...
  if (window.ContinuuuumNav) {
    ContinuuuumNav.mount(document.getElementById('continuuuum-nav-root'), { app: 'project-calendar' });
  }

  var caveShell = window.ContinuuuumCaveShell
    ? window.ContinuuuumCaveShell.init({ tomeId: 'production-calendar-tome', presence: false })
    : null;
  ...
  function renderStories(stories, ctx, schedules) {
    ...
    (schedules || []).forEach(function (sch) {
      ...
      (sch.milestones || []).forEach(function (m) {
        var row = document.createElement('div');
        var storyLinks = (m.continuuuum_story_ids || []).map(function (sid) {
  ...
  async function load() {
    ...
    document.getElementById('subs-list').textContent = subs.length
      ? subs.map(function (s) {
          var cron = s.cron_expr || s.cronExpr || '';
          var human =
            window.CronHumanize && cron ? ' — ' + CronHumanize.describe(cron) : '';
          return s.provider + ' · ' + s.id + (cron ? ' · ' + cron : '') + human;
        }).join('\n')
      : 'No subscriptions';
  ...
  document.getElementById('btn-add-sub').onclick = async function () {
    ...
    var cron =
      (document.getElementById('sub-cron') && document.getElementById('sub-cron').value) ||
      '*/15 * * * *';
    await api('calendar_subscriptions_create', {
      provider: prov,
      cronExpr: cron.trim() || '*/15 * * * *',
  ...
  if (window.CronHumanize) {
    CronHumanize.bindInput(
      document.getElementById('sub-cron'),
      document.getElementById('sub-cron-human')
    );
  }
