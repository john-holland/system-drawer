(function () {
  ...
  if (window.ContinuuuumNav) {
    ContinuuuumNav.mount({ root: '#continuuuum-nav-root', app: 'budget-dashboard' });
  }

  var caveShell = window.ContinuuuumCaveShell
    ? window.ContinuuuumCaveShell.init({ tomeId: 'budget-tome', presence: false })
    : null;
  ...
  document.getElementById('btn-template').onclick = function () {
    ...
    fetch(url, { credentials: 'include' })
      ...
      .then(function (blob) {
        ...
        a.href = objUrl;
        a.download = 'continuuuum-budget-template.json';
