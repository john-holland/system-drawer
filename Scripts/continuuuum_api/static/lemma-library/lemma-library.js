(function (global) {
  ...
  function setActiveNav(page) {
    document.querySelectorAll('#continuuuum-subnav [data-nav]').forEach(a => {
  ...
  async function loadEntry(id) {
    ...
    const libBase = window.ContinuuuumNav
      ? ContinuuuumNav.normalizeLibraryBase(localStorage.getItem('continuuuumLibraryBase') || '', location.origin)
      : (localStorage.getItem('continuuuumLibraryBase') || location.origin + '/library');
    ...
    el.innerHTML =
      ...
      '<a href="/api/deeplink?window=Continuuuum/Lemma+Properties&entryId=' + encodeURIComponent(id) + '" target="_blank">Open in Unity</a>' +
      '</p>' +
    ...
    if (composedHost && global.ContinuuuumLemmaEntry) {
      global.ContinuuuumLemmaEntry.renderInline(composedHost, data.compositionChildren || [], {
        emptyLabel: 'None',
      });
      composedHost.querySelector('.continuuuum-lemma-entry-empty')?.classList.add('muted');
    ...
    document.getElementById('edit-composition')?.addEventListener('click', () => {
      const editor = global.ContinuuuumLemmaPromptEditor || global.ContinuuuumLemmaCompositionEditor;
      if (!editor) return;
      editor.openModal({
        entryId: id,
        parentEntryId: id,
  ...
  function initMultisort() {
    ...
    if (browsePanel && window.ContinuuuumMultisort) {
      multisortBrowse = ContinuuuumMultisort.mount(browsePanel, {
    ...
    if (locPanel && window.ContinuuuumMultisort) {
      multisortLoc = ContinuuuumMultisort.mount(locPanel, {
  ...
  function init() {
    initMultisort();
    loadPosTags('noun');
    if (global.ContinuuuumLemmaCompositionEditor) {
      createModeTabs = global.ContinuuuumLemmaCompositionEditor.mountCreateTabs(
    ...
    if (params.get('libraryBase')) {
      const normalized = window.ContinuuuumNav
        ? ContinuuuumNav.normalizeLibraryBase(params.get('libraryBase'), location.origin)
        : params.get('libraryBase').replace(/\/$/, '');
      localStorage.setItem('continuuuumLibraryBase', normalized);
    }
