(function () {
  ...
  if (window.ContinuuuumNav) {
    ContinuuuumNav.mount({ root: '#continuuuum-nav-root', app: 'legal-tracker' });
  }

  var caveShell = window.ContinuuuumCaveShell
    ? window.ContinuuuumCaveShell.init({ tomeId: 'legal-tracker-tome', presence: false })
    : null;
  ...
  if (window.ContinuuuumUserSession) {
    var stored = localStorage.getItem('continuuuumUserId');
    if (stored) ContinuuuumUserSession.setUserId(stored);
  }

  function userId() {
    if (window.ContinuuuumUserSession) return ContinuuuumUserSession.getUserId();
    return localStorage.getItem('continuuuumUserId') || 'editor';
  }
  ...
  function renderReviewSection(caseData, codeLines) {
    ...
    return (
      ...
      '<p style="margin-top:0.75rem"><a href="/continuuuum_editor/#legal" target="_blank" rel="noopener">Open full document review (Continuuuum editor)</a></p>' +
      '</section>'
    );
  }
