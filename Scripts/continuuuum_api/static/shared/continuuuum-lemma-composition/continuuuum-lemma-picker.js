...
(function (global) {
   ...
   * opts: { language?, partOfSpeech?, description? }
   * On built-in / duplicate 409, returns the existing entry when existingEntryId is present.
   */
  async function resolveOrCreateLemmaEntry(callApi, phrase, excludeIds, opts) {
    ...
    const data = await callApi('GET', `/api/thesaurus/entries?q=${encodeURIComponent(q)}&limit=50`);
    ...
    try {
      ...
      const existingId = err && (
        err.existingEntryId ||
        (err.apiBody && err.apiBody.existingEntryId) ||
        (err.payload && err.payload.existingEntryId)
      );
      if (existingId && !exclude.has(existingId)) {
      ...
      // Last resort: exact term from a fresh search (exact matches sorted first server-side)
      const retry = await callApi('GET', `/api/thesaurus/entries?q=${encodeURIComponent(q)}&limit=50`);
      const retryItems = (retry.items || []).filter((e) => e.id && !exclude.has(e.id));
      const retryAuto = pickAutoSelectLemma(retryItems, q);
      if (retryAuto) return retryAuto;
      // If conflict id was excluded (parent===seed), still return it so caller can skip self-compose
      if (existingId) {
        const existing = await callApi(
          'GET',
          `/api/thesaurus/entries?entryId=${encodeURIComponent(existingId)}`,
  ...
  global.ContinuuuumLemmaPicker = ContinuuuumLemmaPicker;

  if (typeof module !== 'undefined' && module.exports) {
    module.exports = ContinuuuumLemmaPicker;
  }
