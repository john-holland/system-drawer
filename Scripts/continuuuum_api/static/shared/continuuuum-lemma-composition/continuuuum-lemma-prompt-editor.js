...
(function (global) {
  const CompEditor = global.ContinuuuumLemmaCompositionEditor;
  ...
  function mountPromptModalShell(container, opts) {
    ...
    container.innerHTML =
      '<div class="continuuuum-prompt-tabs">' +
      ['Prompt', 'Children', 'Spatial', 'Timing', 'Preview'].map((t, i) =>
        `<button type="button" data-tab="${i}" class="${i === 0 ? 'active' : ''}">${t}</button>`,
      ).join('') +
      '</div>' +
      '<div class="continuuuum-prompt-body">' +
      '<div class="continuuuum-prompt-panel active" data-panel="0">' +
      '<div class="continuuuum-prompt-ace" id="lemma-prompt-ace"></div>' +
      '<div class="continuuuum-prompt-chips" id="lemma-prompt-chips"></div>' +
      '</div>' +
      '<div class="continuuuum-prompt-panel" data-panel="1"><div id="lemma-prompt-children-host"></div></div>' +
      '<div class="continuuuum-prompt-panel" data-panel="2">' +
      '<div class="continuuuum-prompt-grid" id="lemma-prompt-spatial-grid"></div>' +
      '<p class="muted" style="font-size:12px;margin-top:8px">Spatial 4D id: <code id="lemma-spatial-id">—</code></p>' +
      '</div>' +
      '<div class="continuuuum-prompt-panel" data-panel="3">' +
      '<div class="continuuuum-prompt-grid">' +
      ...
      '</div>' +
      '<div class="continuuuum-prompt-panel" data-panel="4">' +
      '<div class="continuuuum-prompt-preview" id="lemma-prompt-preview">Loading…</div>' +
      '<div class="continuuuum-prompt-issues" id="lemma-prompt-issues"></div>' +
      '<table class="preview" id="lemma-prompt-props" style="margin-top:8px;font-size:12px"><tbody></tbody></table>' +
      '</div>' +
      '</div>' +
      '<div class="continuuuum-prompt-actions">' +
    ...
    const panels = container.querySelectorAll('.continuuuum-prompt-panel');
    container.querySelectorAll('.continuuuum-prompt-tabs button').forEach((btn) => {
      btn.onclick = () => {
        container.querySelectorAll('.continuuuum-prompt-tabs button').forEach((b) => b.classList.remove('active'));
    ...
    function renderPatchEditor() {
      ...
      function draw() {
        ...
        rows.innerHTML = entries.length
          ? entries.map(([k, v], i) =>
            `<div class="continuuuum-prompt-patch-row">` +
  ...
  function openModal(opts) {
    ...
    overlay.className = 'continuuuum-prompt-overlay';
    const modal = document.createElement('div');
    modal.className = 'continuuuum-prompt-modal';
    modal.innerHTML = '<h3>Lemma composition</h3>';
    const banner = document.createElement('div');
    banner.className = 'continuuuum-prompt-banner';
  ...
  const ContinuuuumLemmaPromptEditor = {
    openModal,
    buildSavePayload,
    mountPromptModalShell,
  };

  global.ContinuuuumLemmaPromptEditor = ContinuuuumLemmaPromptEditor;

  if (typeof module !== 'undefined' && module.exports) {
    module.exports = ContinuuuumLemmaPromptEditor;
  }
