...
test('conflict actions stay hidden until builtin_conflict', () => {
  ...
  const conflictEl = el('clause-attach-conflict-actions', 'continuuuum-clause-conflict-actions');
  const errEl = el('clause-attach-error', 'continuuuum-clause-dialog-error');
  ...
  const box = {
    ...
    querySelectorAll(sel) {
      if (sel === '.continuuuum-clause-dialog-error') return [errEl];
      if (sel === '.continuuuum-clause-conflict-actions') return [conflictEl];
