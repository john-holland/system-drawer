 ...
 * ContinuuuumMultisort.mount(el, { storageKey, dimensions, onChange })
 */
(function (global) {
  'use strict';
  ...
  function mount(el, options) {
    const storageKey = options.storageKey || 'continuuuum-multisort';
    const dimensions = options.dimensions || [];
    let sortState = loadState(storageKey, dimensions);

    el.innerHTML = '';
    el.classList.add('continuuuum-multisort');
    const title = document.createElement('h3');
    title.textContent = options.title || 'Sort & group';
    el.appendChild(title);
    const list = document.createElement('ul');
    list.className = 'continuuuum-multisort-list';
    el.appendChild(list);
    ...
    function renderList() {
      ...
      ordered.forEach((dim, idx) => {
        ...
        li.className = 'continuuuum-multisort-item';
        li.dataset.id = dim.id;
        li.innerHTML =
          '<span class="continuuuum-multisort-grip">⋮⋮</span>' +
          '<span class="continuuuum-multisort-label"></span>' +
          '<button type="button" class="continuuuum-multisort-dir"></button>' +
          '<input type="checkbox" class="continuuuum-multisort-vis" title="Visible in sort/group">';
        li.querySelector('.continuuuum-multisort-label').textContent = dim.label;
        const dirBtn = li.querySelector('.continuuuum-multisort-dir');
        ...
        const vis = li.querySelector('.continuuuum-multisort-vis');
      ...
      if (global.d3 && global.d3.select) {
        ...
        global.d3.select(list).selectAll('.continuuuum-multisort-item').call(drag);
      }
    }
  ...
  global.ContinuuuumMultisort = { mount, sortItems, groupItems, fieldValue };
