/* Shared Continuuuum script editor — Ace + dashed clause overlays */
(function (global) {
  const Spans = global.ContinuuuumScriptSpans;
  ...
  function renderLemmaBindingMeta(metaEl, binding, snippet) {
    ...
    const LE = global.ContinuuuumLemmaEntry;
    if (!LE) {
  ...
  function finishAceEditorLayout(editor, scriptText) {
    ...
    editor.setReadOnly(!!editor._continuuuumReadOnly);
    const resize = () => {
      try {
        editor.resize(true);
  ...
  const ContinuuuumScriptEditor = {
    ...
    async callApi(method, path, body) {
      ...
      if (global.ContinuuuumUserSession && global.ContinuuuumUserSession.getHeaders) {
        Object.assign(headers, global.ContinuuuumUserSession.getHeaders());
      }
    ...
    mount(el, options) {
      ...
      options = options || {};
      el.innerHTML = '';
      el.classList.add('continuuuum-script-editor-host');
      const toolbar = document.createElement('div');
      toolbar.className = 'continuuuum-script-toolbar';
      ...
      autoFixLemmaAnchorsBtn.className = 'continuuuum-auto-fix-lemma-anchors-btn';
      ...
      suggestionsEl.className = 'continuuuum-clause-suggestions';
      ...
      editorEl.id = 'continuuuum-ace-editor';
      ...
      if (aceLoaded) {
        ...
        editor._continuuuumReadOnly = readOnly;
      ...
      const scheduleRefresh = () => {
        ...
        if (typeof document !== 'undefined' && document.querySelector('.continuuuum-clause-overlay')) return;
        if (inst._overlayRaf) cancelAnimationFrame(inst._overlayRaf);
        inst._overlayRaf = requestAnimationFrame(() => {
          inst._overlayRaf = null;
          ContinuuuumScriptEditor.renderOverlays(inst, inst.options);
      ...
      attachBtn.onclick = () => {
        if (!global.ContinuuuumClauseSelector) {
          alert('ContinuuuumClauseSelector not loaded');
          return;
        }
        const clauseRef = global.ContinuuuumClauseSelector.fromEditorSelection(inst);
        global.ContinuuuumClauseSelector.openAttachDialog(clauseRef, {
          draftScriptId: options.draftScriptId,
          draftEpisodeId: options.draftEpisodeId || options.draftId,
          scriptText: ContinuuuumScriptEditor.getValue(inst),
          onAttached: async () => {
    ...
    async fetchClauseSuggestions(selectionText, inst) {
      ...
      const callApi = (method, path, body) => ContinuuuumScriptEditor.callApi(method, path, body);
      ...
      const scriptText = ContinuuuumScriptEditor.getValue(inst);
    ...
    renderClauseSuggestions(inst) {
      ...
      if (!inst._debouncedSuggestions) {
        inst._debouncedSuggestions = debounce((text) => {
          ContinuuuumScriptEditor._loadClauseSuggestions(inst, text);
        }, 200);
      }
      if (inst._lastSuggestionText !== selectionText) {
        el.innerHTML = '<span class="continuuuum-clause-suggestions-hint">Loading…</span>';
    ...
    async _loadClauseSuggestions(inst, selectionText) {
      ...
      try {
        ...
        items.forEach((tpl) => {
          ...
          btn.className = `continuuuum-clause-suggestion-btn continuuuum-clause-suggestion-${kind}`;
    ...
    renderClausePanel(inst) {
      ...
      heading.className = 'continuuuum-clause-panel-heading';
      ...
      list.className = 'continuuuum-clause-panel-list';
      bindings.forEach((b) => {
        const card = document.createElement('div');
        card.className = 'continuuuum-clause-card';
        const kind = b.bindingKind || b.binding_kind || 'property';
        const liveSlice = text.substring(b.charStart, b.charEnd);
        const snippet = liveSlice || b.selectionText || b.selection_text || '—';
        card.innerHTML =
          `<span class="continuuuum-clause-kind continuuuum-clause-kind-${kind}">${kind}</span>` +
          `<span class="continuuuum-clause-snippet">"${snippet.slice(0, 40)}" [${b.charStart}, ${b.charEnd})</span>`;
        const meta = document.createElement('span');
        meta.className = 'continuuuum-clause-meta';
        if (kind === 'lemma') {
        ...
        if (!inst.readOnly && global.ContinuuuumClauseSelector) {
          if (kind === 'lemma') {
            const entryId = b.entryId || b.entry_id || b.propertyValue || b.property_value;
            if (entryId && global.ContinuuuumClauseSelector.openLemmaEntryDialog) {
              const editBtn = document.createElement('button');
              editBtn.type = 'button';
              editBtn.textContent = 'Edit lemma';
              editBtn.className = 'continuuuum-clause-edit-btn';
              editBtn.onclick = () => {
                global.ContinuuuumClauseSelector.openLemmaEntryDialog({
                  entryId,
            ...
            editBtn.className = 'continuuuum-clause-edit-btn';
            editBtn.onclick = () => {
              global.ContinuuuumClauseSelector.openEditDialog(b, {
                draftEpisodeId: options.draftEpisodeId || options.draftId,
                draftScriptId: options.draftScriptId,
                scriptText: text,
                permissions: options.permissions,
        ...
        if (kind === 'lemma' && global.ContinuuuumLemmaPromptEditor) {
          const entryId = b.entryId || b.entry_id || b.propertyValue || b.property_value;
          if (entryId) {
            ...
            compBtn.className = 'continuuuum-clause-edit-btn';
            compBtn.style.marginLeft = '4px';
            compBtn.onclick = () => {
              global.ContinuuuumLemmaPromptEditor.openModal({
                entryId,
    ...
    async autoFixLemmaAnchors(inst) {
      ...
      if (!Spans || !Spans.mismatchedLemmaBindingsInRange || !Spans.proposeLemmaReanchor) {
        alert('ContinuuuumScriptSpans not loaded');
        return;
      }
      const options = inst.options || {};
  ...
  const ContinuuuumChangeListModal = {
    open(changeListId, data, callbacks) {
      ...
      const ackExtra = (global.ContinuuuumScriptAck && data && data._changeListForAck)
        ? global.ContinuuuumScriptAck.buildChangeListAckItems(data._changeListForAck)
        : [];
      const state = {
      ...
      function unacknowledgedRequired(items) {
        if (global.ContinuuuumScriptAck && global.ContinuuuumScriptAck.unacknowledgedRequired) {
          return global.ContinuuuumScriptAck.unacknowledgedRequired(items);
        }
  ...
  global.ContinuuuumScriptEditor = ContinuuuumScriptEditor;
  global.ContinuuuumChangeListModal = ContinuuuumChangeListModal;
