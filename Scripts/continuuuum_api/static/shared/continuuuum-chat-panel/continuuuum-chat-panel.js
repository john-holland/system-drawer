...
(function (global) {
  ...
  function renderContent(text) {
    ...
    safe = safe.replace(/\[([^\]]+)\]\(([^)]+)\)/g, function (_, label, url) {
      return '<a href="' + esc(url) + '" class="continuuuum-chat-link" target="_blank" rel="noopener">' + esc(label) + '</a>';
    });
    safe = safe.replace(/(https?:\/\/[^\s<]+)/g, function (url) {
      return '<a href="' + esc(url) + '" class="continuuuum-chat-link" target="_blank" rel="noopener">' + esc(url) + '</a>';
    });
    return safe;
  }
  ...
  function headers(extra) {
    if (global.ContinuuuumUserSession) {
      return global.ContinuuuumUserSession.getHeaders(Object.assign({ 'Content-Type': 'application/json' }, extra || {}));
    }
  ...
  function renderMessages(box, messages) {
    ...
    box.innerHTML = items.map(function (m) {
      var cls = m.type === 'system' ? 'continuuuum-chat-msg continuuuum-chat-msg--system' : 'continuuuum-chat-msg';
      return '<div class="' + cls + '"><span class="continuuuum-chat-sender">' + esc(m.sender) + '</span> ' +
  ...
  function mount(el, options) {
    ...
    var sender = options.sender || (global.ContinuuuumUserSession ? global.ContinuuuumUserSession.getUserId() : 'user');
    var pollMs = options.pollMs || 4000;
    var useTome = options.useTome !== false;
    var mountId = 'chat-' + Math.random().toString(36).slice(2);

    el.className = (el.className ? el.className + ' ' : '') + 'continuuuum-chat-panel continuuuum-chat-panel--inventory';
    el.innerHTML =
      '<div class="continuuuum-chat-panel-header"><strong>Chat</strong></div>' +
      '<div class="continuuuum-chat-messages" id="' + mountId + '-msgs"></div>' +
      '<div class="continuuuum-chat-compose">' +
        '<textarea class="continuuuum-chat-input" rows="2" placeholder="Message"></textarea>' +
        '<button type="button" class="continuuuum-chat-send">Send</button>' +
      '</div>';

    var msgBox = el.querySelector('#' + mountId + '-msgs');
    var input = el.querySelector('.continuuuum-chat-input');
    var sendBtn = el.querySelector('.continuuuum-chat-send');
  ...
  global.ContinuuuumChatPanel = { mount: mount, renderContent: renderContent };
