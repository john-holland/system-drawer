(function (global) {
  ...
  var T = function () { return global.ContinuuuumSqlViewerTemplates; };
  ...
  SqlViewerTome.prototype._wireStaticEvents = function () {
    ...
    if (global.ContinuuuumUserSession) {
      global.ContinuuuumUserSession.onChange(function () { self.boot(); });
    }
  };

  SqlViewerTome.prototype.isAuthenticated = function () {
    if (!global.ContinuuuumUserSession) return false;
    var uid = (global.ContinuuuumUserSession.getUserId() || '').trim();
  ...
  SqlViewerTome.prototype.runQuery = function () {
    ...
    var check = global.ContinuuuumSqlSafety.validateClientSql(sql);
    if (!check.ok) {
  ...
  SqlViewerTome.prototype.boot = function () {
    var self = this;
    var gatePromise = Promise.resolve();
    if (!this.isAuthenticated() && global.ContinuuuumUserSession && global.ContinuuuumUserSession.ensurePresent) {
      this.transition('gate');
      gatePromise = global.ContinuuuumUserSession.ensurePresent({ title: 'SQL Viewer — sign in required' });
    ...
    return gatePromise.then(function () {
      ...
      self.transition('booting');
      return Promise.all([self.loadSchema(), self.loadRecipes()])
        .then(function () {
          self.state = 'ready';
          self.setExecMsg('', '');
        })
        .catch(function (e) {
          if (e.status === 401 || (e.body && e.body.code === 'auth_required')) {
            self.transition('gate');
            if (global.ContinuuuumUserSession && global.ContinuuuumUserSession.ensurePresent) {
              return global.ContinuuuumUserSession.ensurePresent({ title: 'SQL Viewer — sign in required' })
                .then(function () { return self.boot(); });
            }
          } else {
            self.setExecMsg(e.message, 'error');
            self.state = 'ready';
  ...
  global.ContinuuuumSqlViewerTome = { mount: mount, SqlViewerTome: SqlViewerTome };
