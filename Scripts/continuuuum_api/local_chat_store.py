...
def ensure_schema(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS chat_rooms (
            ...
            continuuuum_story_id TEXT,
            continuuuum_table_read_session_id TEXT,
            participants_json TEXT NOT NULL DEFAULT '[]',
            watchers_json TEXT NOT NULL DEFAULT '[]',
            created_at TEXT NOT NULL
        );
        CREATE INDEX IF NOT EXISTS idx_chat_rooms_story ON chat_rooms(continuuuum_story_id);
        CREATE INDEX IF NOT EXISTS idx_chat_rooms_table_read ON chat_rooms(continuuuum_table_read_session_id);
...
def _room_row_to_client(row: sqlite3.Row, messages: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    return {
        ...
        "continuuuum_story_id": row["continuuuum_story_id"],
        "continuuuum_table_read_session_id": row["continuuuum_table_read_session_id"],
...
def find_story_room(conn: sqlite3.Connection, story_id: str) -> dict[str, Any] | None:
    ensure_schema(conn)
    row = conn.execute(
        "SELECT * FROM chat_rooms WHERE continuuuum_story_id = ? LIMIT 1",
        (story_id,),
    ).fetchone()
...
def ensure_story_room(
    ...
    existing = conn.execute(
        "SELECT id FROM chat_rooms WHERE continuuuum_story_id = ? LIMIT 1",
        (story_id,),
    ).fetchone()
    participants = sorted(
    ...
    conn.execute(
        """INSERT INTO chat_rooms
           (id, name, room_type, continuuuum_story_id, participants_json, watchers_json, created_at)
           VALUES (?, ?, 'story', ?, ?, ?, ?)""",
...
def ensure_table_read_room(
    ...
    ensure_schema(conn)
    existing = conn.execute(
        "SELECT id FROM chat_rooms WHERE continuuuum_table_read_session_id = ? LIMIT 1",
        (session_id,),
    ).fetchone()
    ...
    conn.execute(
        """INSERT INTO chat_rooms
           (id, name, room_type, continuuuum_table_read_session_id, participants_json, watchers_json, created_at)
           VALUES (?, ?, 'table_read', ?, ?, '[]', ?)""",
