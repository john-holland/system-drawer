-- Continuuuum SQL Viewer recipe queries (read-only)
-- Metadata lines use: -- @recipe id=... label="..." description="..."

-- @recipe id=schema_tables label="All user tables" description="List tables and views in continuuuum.db"
SELECT name, type
FROM sqlite_master
...
-- @recipe id=credits_list_entries label="Credits list entries" description="Credits sections and entries including hidden (guild review)"
SELECT l.id AS list_id, l.title AS list_title, s.title AS section_title, s.scroll_speed AS section_speed,
       e.id AS entry_id, e.full_name, e.nick_name, e.show_full_name, e.show_nickname,
       e.company, e.years, e.rights_marks, e.source_user_id, e.source_kind, e.sort_order
FROM credits_lists l
JOIN credits_sections s ON s.list_id = l.id
JOIN credits_entries e ON e.section_id = s.id
ORDER BY l.updated_at DESC, s.sort_order, e.sort_order
LIMIT 200;

-- @recipe id=credits_warehouse_history label="Credits warehouse history" description="Append-only credits audit for guild issues"
...
WHERE e.show_full_name = 0 AND e.show_nickname = 0
