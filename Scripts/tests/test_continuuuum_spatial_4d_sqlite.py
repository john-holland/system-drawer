"""
SQLite integration tests for Continuuuum schema SQL (episodes + spatial_4d + spatial_4d_history).

These validate the SQL shipped under Scripts/ against SQLite (same engine as continuuuum_api/server.py).

Run from Drawer 2 repo (install pytest if needed):
  cd Scripts
  pytest tests/test_continuuuum_spatial_4d_sqlite.py -v
"""
...
def conn_episodes_and_spatial(tmp_path: Path) -> sqlite3.Connection:
    """Fresh DB with continuuuum_episodes_schema + continuuuum_spatial_4d_schema applied."""
    db_path = tmp_path / "continuuuum_test.db"
    conn = sqlite3.connect(str(db_path))
    conn.execute("PRAGMA foreign_keys = ON")
    conn.executescript(_read_sql("continuuuum_episodes_schema.sql"))
    conn.executescript(_read_sql("continuuuum_spatial_4d_schema.sql"))
    conn.commit()
    return conn
