...
def test_parse_route():
    svc, structural = parse_route("continuum:stories/list")
    assert svc == "continuum"
...
def test_manifest_loads_handlers():
    ...
    assert manifest.get("service") == "continuum"
...
def test_cave_route_unknown():
    ...
    r = client.post("/cave/route", json={"route": "continuum:does/not/exist", "trace_id": "t1"})
...
def test_cave_route_resaurce_proxy():
    ...
    with patch("cave.resaurce_proxy.proxy_cave_route", side_effect=mock_proxy):
        r = client.post(
            "/cave/route",
            json={
                "route": "continuum:production/budget/list",
                "payload": {},
