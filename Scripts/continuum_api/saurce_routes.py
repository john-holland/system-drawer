...
def _product_row(row: sqlite3.Row) -> dict[str, Any]:
    ...
    return {
        ...
        "continuumAssetId": d.get("continuum_asset_id"),
...
def _ledger(conn: sqlite3.Connection, entry_type: str, product_id: str | None, **kw: Any) -> str:
    ...
    try:
        from continuum_api.production_journal_bridge import mirror_saurce_ledger_to_resaurce
    except ImportError:
        ...
        if request.method == "POST":
            ...
            conn.execute(
                """INSERT INTO saurce_products
                   (id, slug, name, type, description, continuum_asset_id, lemma_package_id,
