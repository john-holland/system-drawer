...
(function (global) {
  ...
  function linkHtml(entry, opts) {
    ...
    const cls = ['continuum-lemma-entry-link', opts.className].filter(Boolean).join(' ');
    ...
    if (opts.showId) {
      html += ` <code class="continuum-lemma-entry-id">${escHtml(n.entryId)}</code>`;
    }
    return html;
  }
  ...
  function chipHtml(entry, opts) {
    ...
    return linkHtml(entry, {
      ...opts,
      className: ['continuum-lemma-entry-chip', opts.className].filter(Boolean).join(' '),
    });
  }
  ...
  function createLink(entry, opts) {
    ...
    wrap.className = 'continuum-lemma-entry-wrap';
    if (!n) {
    ...
    a.className = ['continuum-lemma-entry-link', opts.className].filter(Boolean).join(' ');
    ...
    if (opts.showId) {
      ...
      code.className = 'continuum-lemma-entry-id';
  ...
  function renderInline(parent, entries, opts) {
    ...
    if (!items.length) {
      ...
      empty.className = 'continuum-lemma-entry-empty';
  ...
  function renderList(parent, entries, opts) {
    ...
    ul.className = ['continuum-lemma-entry-list', opts.className].filter(Boolean).join(' ');
    const items = (entries || []).map(normalize).filter(Boolean);
    if (!items.length) {
      const li = document.createElement('li');
      li.className = 'continuum-lemma-entry-empty';
