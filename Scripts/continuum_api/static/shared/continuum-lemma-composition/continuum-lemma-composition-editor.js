...
(function (global) {
  const Picker = global.ContinuumLemmaPicker;
  ...
  function renderChildList(listEl, children, handlers) {
    ...
    children.forEach((c, idx) => {
      ...
      if (global.ContinuumLemmaEntry) {
        labelWrap.appendChild(global.ContinuumLemmaEntry.createLink(c, { showId: true }));
      } else {
        labelWrap.innerHTML =
  ...
  function mountEditorShell(container, opts) {
    ...
    container.innerHTML =
      '<div class="continuum-comp-editor">' +
      ...
      '<ul class="continuum-comp-list"></ul>' +
      '<div class="continuum-comp-actions">' +
    ...
    const listEl = container.querySelector('.continuum-comp-list');
    ...
    container.querySelector('.comp-recomb').onclick = () => {
      ContinuumLemmaCompositionEditor.openRecombobulateModal({
  ...
  function openRecombobulateModal(opts) {
    ...
    overlay.className = 'continuum-comp-overlay';
    overlay.innerHTML =
      '<div class="continuum-comp-modal">' +
    ...
    function renderIssues() {
      ...
      issuesEl.innerHTML = issues.map((issue) => {
        ...
        const diff = (issue.storedText != null || issue.currentText != null)
          ? `<div class="continuum-recomb-diff">Stored: "${escHtml(issue.storedText || '')}"<br>Current: "${escHtml(issue.currentText || '')}"</div>`
          : '';
        return `<div class="continuum-recomb-issue${issue.requiresAck ? ' requires-ack' : ''}">` +
  ...
  function openModal(opts) {
    if (global.ContinuumLemmaPromptEditor && global.ContinuumLemmaPromptEditor.openModal) {
      return global.ContinuumLemmaPromptEditor.openModal(opts);
    }
    opts = opts || {};
    const overlay = document.createElement('div');
    overlay.className = 'continuum-comp-overlay';
    const modal = document.createElement('div');
    modal.className = 'continuum-comp-modal';
    modal.innerHTML = '<h3>Edit lemma composition</h3>';
    const host = document.createElement('div');
    modal.appendChild(host);
    const closeRow = document.createElement('div');
    closeRow.className = 'continuum-comp-actions';
  ...
  const ContinuumLemmaCompositionEditor = {
    mountInline,
    openModal,
  ...
  global.ContinuumLemmaCompositionEditor = ContinuumLemmaCompositionEditor;

  if (typeof module !== 'undefined' && module.exports) {
    module.exports = ContinuumLemmaCompositionEditor;
  }
