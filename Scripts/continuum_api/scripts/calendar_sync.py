...
def build_ics_events(conn: sqlite3.Connection, story_id: str | None = None) -> str:
    ...
    lines = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Continuum//Agile PM//EN"]
    for r in rows:
        uid = r["id"]
...
def _sync_google(conn: sqlite3.Connection, sub: sqlite3.Row, ics: str) -> dict:
    ...
    for line_block in ics.split("BEGIN:VEVENT"):
        ...
        service.events().insert(
            ...
            body={
                "summary": summary,
                "description": "Continuum agile sync",
...
def _sync_outlook(conn: sqlite3.Connection, sub: sqlite3.Row, ics: str) -> dict:
    ...
    for line_block in ics.split("BEGIN:VEVENT"):
        ...
        body = json.dumps(
            {
                "subject": summary,
                "body": {"contentType": "text", "content": "Continuum agile sync"},
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Continuum calendar sync")
    ...
    db_path = args.db or os.environ.get("CONTINUUM_DB", str(Path(__file__).resolve().parents[2] / "continuum.db"))
