...
def ensure_schema(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS schedule_milestones (
            ...
            start_date TEXT,
            end_date TEXT,
            continuum_story_ids_json TEXT NOT NULL DEFAULT '[]',
...
def _milestone_row(row: sqlite3.Row) -> dict[str, Any]:
    return {
        ...
        "continuum_story_ids": json.loads(row["continuum_story_ids_json"] or "[]"),
    }
    ...
    for m in milestones:
        conn.execute(
            """INSERT INTO schedule_milestones
               (id, schedule_id, label, start_date, end_date, continuum_story_ids_json)
               VALUES (?, ?, ?, ?, ?, ?)""",
            (
                _new_id("ms"),
                schedule_id,
                ...
                json.dumps(m.get("continuum_story_ids") or m.get("continuumStoryIds") or []),
            ),
        )
    conn.commit()
...
def update_milestone_stories(
    ...
    if not row:
        return None
    conn.execute(
        "UPDATE schedule_milestones SET continuum_story_ids_json = ? WHERE id = ?",
