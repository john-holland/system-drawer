...
Architecture review of Continuum, resaurce, and log-view-machine UI mechanisms.  
**Date:** 2026-06-30 · **Scope:** full stack (Continuum + resaurce + log-view-machine canonical model)
...
| **Lemma Library** | `/lemma-library` | Vanilla + shared widgets | `lemma` + hash subnav | `ContinuumUserSession` | `lemma-tome` | Direct `/api/thesaurus/*` | `browseItems`, hash router | Audit only |
...
| **Script Output** | `/script-output` (Vite `:5174`) | Vanilla (Vite host) | `hub` (mislabeled) | Session | None | `ContinuumScriptOutput.api()` → `/api/*` | `_state` object | Audit only |
...
| **Table Read** | `/table-read` | Vanilla + Socket.IO | `table-read` | Session | `table-read-tome` | **Hybrid:** REST + tome messages + socket | `state` in `continuum-table-read.js` | Partial |
...
|---------|----------|-------|-----|-----------|---------------|
| **Unity Script Editor** | `Assets/Continuum/Editor/ScriptEditor/WebView/` | Vanilla + Unity bridge | None | C# `ContinuumEditorApiClient` | D |
| **Continuum WebGL editor** | `continuum/library/continuum_editor_webgl/` | Unity WebGL | Unity | `?apiBase=` | D |
...
| Module | Path | Role |
|--------|------|------|
| `continuum-nav.js` | `static/shared/continuum-nav/` | Cross-app header, chat toggle, dev user switcher |
| `continuum-user-session.js` | `static/shared/continuum-user-session/` | `X-User-ID`, `X-Admin` headers |
| `continuum-cave-shell.js` | `static/shared/continuum-cave-shell/` | RobotCopy, presence, preorder gate |
| `continuum-tome-bootstrap.js` | same | Thin wrapper: `init({ tomeId })` + optional `onReady` |
| Shared widgets | `continuum-script-editor`, `continuum-lemma-*`, `continuum-table-read`, `continuum-chat-panel` | Reused across hub, lemma, table-read, script-output |
...
2. **~10 duplicated `fetch`/`api()` wrappers** — no shared `ContinuumApiClient`.
3. **Tome cargo-cult** — 8+ pages mount `ContinuumTomeBootstrap` but never call `RobotCopy` (presence/gate side effects only).
...
| Capability | log-view-machine | resaurce | Continuum (Drawer 2) |
|------------|------------------|----------|----------------------|
...
| Single-server target | Documented | Separate `:3456` Cave | Flask `:5050` (see `CONTINUUM_SINGLE_SERVER.md`) |

### 2.3 Dual Cave problem

```mermaid
flowchart TB
  subgraph continuum [Continuum Flask :5050]
    Nav[continuum-nav static apps]
  ...
  CaveTS -.->|"reference only"| continuum
  VSM -.->|"inventory MF future"| resaurce
```

**Convergence gap:** Continuum YAML tomes describe routing that Python does not enforce. Resaurce manifest is authoritative for domain + LVM2. log-view-machine is not bundled in any Continuum static page.

### 2.4 `_dispatch_tome_message` drift risk

File: `Scripts/continuum_api/cave_routes.py` (lines 114–241).
...
| State ownership | `ContinuumSqlViewerTome` class (`model`, `state` string) |
...
| Cons | Separate service; envelope learning curve; not wired to most Continuum pages |
...
**Cave→Tome→LVM is recommended for trace-sensitive, multi-step, cross-service flows — not as a blanket wrapper around every Continuum vanilla SPA.**
...
- Standardize **Tier C** baseline for all production Continuum agile/editor UIs.
...
| Stop | Start |
|------|-------|
| Mounting `ContinuumTomeBootstrap` without any `RobotCopy` usage | Tier C: `ContinuumNav` + `ContinuumUserSession` only |
| Growing `_dispatch_tome_message` by hand | Tome YAML → dispatch registration (Spike 1) |
| Per-app `fetch` wrappers | Shared `ContinuumApiClient` (Spike 2) |
...
| **Spike 3: Table-read path unify** | `continuum-table-read.js` | Session open + chat use either REST or tome, not both; document chosen path in RFC |

### 5.4 Open questions (defer to follow-up)

1. LVM2 bridge on Continuum Flask mutations — local append vs forward to resaurce `:3456`?
2. When to merge resaurce Cave into single-server `:5050` per `CONTINUUM_SINGLE_SERVER.md`?
3. Bundle log-view-machine in Continuum for one greenfield Tier A pilot?
...
|----------|------|--------|--------|
| P0 | Shared `ContinuumApiClient` | S | Removes duplication; enables trace headers everywhere |
...
| P2 | LVM2 bridge for Tier B Continuum mutations | L | Pact/trace parity with resaurce |
| P2 | log-view-machine pilot (one MF surface) | L | Validates Tier A path |
| P3 | Full single-server merge (Continuum + serve_library + resaurce proxy) | XL | Ops simplicity |
...
| Area | Path |
|------|------|
| Cave config | `Scripts/continuum_api/cave/cave.yaml` |
| Tome YAML | `Scripts/continuum_api/cave/tomes/*.yaml` |
| CaveRobit | `Scripts/continuum_api/cave/cave-robit.yaml` |
| Loader | `Scripts/continuum_api/cave_loader.py` |
| Dispatch | `Scripts/continuum_api/cave_routes.py` |
| Nav | `Scripts/continuum_api/static/shared/continuum-nav/continuum-nav.js` |
| Shell | `Scripts/continuum_api/static/shared/continuum-cave-shell/` |
...
| Single-server target | `continuum/CONTINUUM_SINGLE_SERVER.md` |
