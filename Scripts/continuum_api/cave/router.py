...
def _trace_id(body: dict[str, Any]) -> str:
    ...
    if tid:
        return str(tid)
    return f"continuum_{uuid.uuid4().hex[:12]}"
...
def handle_cave_route(
    ...
    if message and not route_raw:
        ...
        explicit_service = manifest.get("service") or "continuum"
    elif route_raw:
    ...
    if not spec:
        return {"ok": False, "error": "unknown_route", "route": f"continuum:{structural}"}
