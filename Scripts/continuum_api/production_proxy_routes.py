...
try:
    from continuum_api.local_production_store import (
...
def register_production_proxy_routes(app, get_conn: GetConn) -> None:
    ...
    def production_budget_template():
        try:
            from continuum_api.scripts.sheets_publish import budget_template_payload
        except ImportError:
        ...
        if download:
            ...
            return Response(
                ...
                headers={
                    "Content-Disposition": 'attachment; filename="continuum-budget-template.json"',
                },
            )
    ...
    def production_schedules_create_with_stories():
        ...
        for idx, m_in in enumerate(milestones_in):
            ...
            story_ids = list(m_row.get("continuum_story_ids") or [])
    ...
    def production_milestone_stories(schedule_id: str, milestone_id: str):
        body = request.get_json(silent=True) or {}
        story_ids = body.get("continuumStoryIds") or body.get("story_ids") or []
        out, status = _cave(
            "production/schedule/milestone/stories",
            {
                "schedule_id": schedule_id,
