-- Optional seed: materialize Unity built-in thesaurus URNs into continuum.db after core schema.
-- Operational rule: continuum ETL and API must NOT mint thesaurus_entries.id values with prefix
--   urn:unity:continuum:builtin:v1:
-- (see C# VocabularyLanguageEncoding.BuiltInUrnPrefix in Drawer 2 / Assets/Continuum).
--
-- Requires: languages row for code 'en' (or change :lang below). Run after continuum_thesaurus_schema.sql.
...
-- SELECT
--   'urn:unity:continuum:builtin:v1:en/det/the',
...
--   '1.0',
--   datetime('now')
-- WHERE EXISTS (SELECT 1 FROM thesaurus_entries WHERE id = 'urn:unity:continuum:builtin:v1:en/det/the');
